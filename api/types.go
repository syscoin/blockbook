		...
		for i := range a {
			...
			if len(bh.Tokens) > 0 {
				for i, token := range bh.Tokens {
					var bhaToken *TokenBalanceHistory
					var ok bool
					if bha.Tokens == nil {
					...
					(*big.Int)(sentSat).Add((*big.Int)(sentSat), (*big.Int)(&token.SentSat))
					(*big.Int)(receivedSat).Add((*big.Int)(receivedSat), (*big.Int)(&token.ReceivedSat))
				}
			}
		}
		if bha.Txs > 0 {
