...
func (w *Worker) GetAddress(address string, page int, txsOnPage int, option AccountDetails, filter *AddressFilter) (*Address, error) {
	...
	if ba.AssetBalances != nil && option > AccountDetailsBasic {
		...
		for k, v := range ba.AssetBalances {
			totalReceived := bchain.ReceivedSatFromBalances(&v.BalanceAssetSat, &v.SentAssetSat)
	...
	if w.chainType == bchain.ChainBitcoinType {
		...
		if len(ta.TokenTransfers) > 0 {
			...
			if bytes.Equal(addrDesc, tattAddrFromDesc) {
				(*big.Int)(bh.Tokens[0].SentSat).Add((*big.Int)(bh.Tokens[0].SentSat), (*big.Int)(tatt.Value))
